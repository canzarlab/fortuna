...
void MasterProcessor::processReads() {
 bamh = createPseudoBamHeaderTrans(index); 
  ...
  if (!opt.batch_mode && !opt.bus_mode) {
	...
	MT_Aligner* AL = (MT_Aligner*)ALIGNER;
	if (AL->opt.bam != "")
	{
		AL->opt.bamfp = sam_open(AL->opt.bam.c_str(), "wb"); // LUKA: don't write the BAM file
    	int r = sam_hdr_write(AL->opt.bamfp, bamh); 
	}

	// ACHTUNG: This is where reads get processed.

    std::vector<std::thread> workers;
    for (int i = 0; i < opt.threads; i++) {
      workers.emplace_back(std::thread(ReadProcessor(index,opt,tc,*this))); // ACHTUNG: thread aligner
    }
	...
	if (AL->opt.bam != "") sam_close(AL->opt.bamfp);
...
void MasterProcessor::processAln(const EMAlgorithm& em, bool useEM = true) {
  ...
  if (opt.pseudobam) {
    if (opt.genomebam) {
      ...
      bamh = createPseudoBamHeaderTrans(index);
      bamfp = sam_open(bamfn.c_str(), "wb"); // LUKA: don't write the BAM file
  ...
  std::cerr << "[  bam] processing pseudoalignments ... "; std::cerr.flush();
  ...
  */
  ...
  std::vector<std::thread> workers;
  for (int i = 0; i < opt.threads; i++) {
    workers.emplace_back(std::thread(AlnProcessor(index,opt,*this, em, model, useEM, i)));
  }
...
// ACHTUNG: batch alignments from ReadProcessor go here
void MasterProcessor::update(const std::vector<int>& c, const std::vector<std::vector<int> > &newEcs, 
                            ...
                            int n, std::vector<int>& flens, std::vector<int> &bias, const PseudoAlignmentBatch& pseudobatch, std::vector<BUSData> &bv, std::vector<std::pair<BUSData, std::vector<int32_t>>> newBP, int *bc_len, int *umi_len,  std::vector<std::pair<const char*, int>>& seqs, std::vector<std::pair<const char*, int>>& names, std::vector<std::pair<const char*, int>>& quals, int id, int local_id) {
  ...
  if (opt.pseudobam) {
    PseudoAlignmentBatch BB; 
    {
    std::lock_guard<std::mutex> lock(this->writer_lock);
    BB = PseudoAlignmentBatch(std::move(pseudobatch));
    }
    processBufferTrans(BB, seqs, names, quals);
    ...
    while (true) {
      vector<PseudoAlignmentBatch>::iterator min_it;
      { 
      std::lock_guard<std::mutex> lock(this->writer_lock);	
      ...
      min_it = std::min_element(pseudobatch_stragglers.begin(), pseudobatch_stragglers.end(), 
      ...
      processBufferTrans(*min_it, seqs, names, quals);

      //writePseudoAlignmentBatch(pseudobatchf_out, *min_it); // ACHTUNG: instead of writing, maybe send to MT_Aligner...
...
// ACHTUNG: big hack
void MasterProcessor::processBufferTrans(PseudoAlignmentBatch& pseudobatch, std::vector<std::pair<const char*, int>>& seqs, std::vector<std::pair<const char*, int>>& names, std::vector<std::pair<const char*, int>>& quals) {
...
// LUKA: modify here!!!
// clean
void MasterProcessor::writePseudoBam(const std::vector<bam1_t> &bv, int id) {
  MT_Aligner* A = (MT_Aligner*)ALIGNER;
  if (A->KallistoCallback(bamh, bv, id)) return;	
  std::lock_guard<std::mutex> lock(this->writer_lock);	
...
// ACHTUNG: this gets created by MP threads
...
void ReadProcessor::operator()() {
  while (true) {
    ...
    processBuffer(); // ACHTUNG: batch start aligning.

    // update the results, MP acquires the lock
    std::vector<BUSData> tmp_v{};
    mp.update(counts, newEcs, ec_umi, new_ec_umi, paired ? seqs.size()/2 : seqs.size(), flens, bias5, pseudobatch, tmp_v, std::vector<std::pair<BUSData, std::vector<int32_t>>>{}, nullptr, nullptr, seqs, names, quals, id, local_id); // ACHTUNG: batch done aligning.
    clear();
  }
}

// ACHTUNG: this is where alignments are made
void ReadProcessor::processBuffer() {
  ...
  for (int i = 0; i < seqs.size(); i++) {
    ...
    if (mp.opt.pseudobam) {
      PseudoAlignmentInfo info; // ACHTUNG: info of the alignment
...
void BUSProcessor::operator()() {
  while (true) {
    ...
    /*mp.update(counts, newEcs, ec_umi, new_ec_umi, seqs.size() / mp.opt.busOptions.nfiles , flens, bias5, pseudobatch, bv, newB, &bc_len[0], &umi_len[0], id, local_id);*/ // LUKA: comment
    clear();
  }
}
